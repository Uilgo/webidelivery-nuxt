 ...
 *
 * Modal de confirmação para exclusão de grupos de adicionais.
 * Padrão consistente com CategoriaDeleteModal e ProdutoDeleteModal.
 * Requer digitação do nome do grupo para confirmar exclusão.
 */
...
// Composables
const { remove, deleting, actionError } = useGruposAdicionaisActions();
const { refresh } = useGruposAdicionaisFetch();
...
// Verifica se o grupo pode ser excluído (sempre true, mas com aviso se tiver adicionais)
...
const isConfirmacaoValida = computed(() => {
	if (!props.grupo) return false;
	return confirmacaoNome.value.trim() === props.grupo.nome.trim();
});
...
const warningMessage = computed(() => {
	...
	const adicionaisCount = props.grupo.adicionais_count ?? 0;

	if (adicionaisCount > 0) {
		return `Este grupo possui ${adicionaisCount} adicional(is) cadastrado(s). Ao excluir o grupo, TODOS os ${adicionaisCount} adicional(is) serão permanentemente excluídos do sistema.`;
	}

	return "Esta ação não pode ser desfeita. O grupo será permanentemente removido do sistema.";
});
...
const handleConfirm = async (): Promise<void> => {
	if (!props.grupo || !canDelete.value) return;

	try {
		const success = await remove(props.grupo.id);

		if (success) {
			toast.add({
				title: "Grupo excluído",
				description: `O grupo "${props.grupo.nome}" foi excluído com sucesso`,
				color: "success",
				duration: 4000,
			});
...
/**
 * Handler para copiar o nome do grupo
 */
const copiarNome = async (): Promise<void> => {
	if (!props.grupo) return;

	try {
		await navigator.clipboard.writeText(props.grupo.nome);
		copiado.value = true;
		...
		toast.add({
			title: "Nome copiado",
			description: "O nome do grupo foi copiado para a área de transferência",
			color: "success",
			duration: 2000,
		});
	} catch (error) {
		...
		toast.add({
			...
			description: "Não foi possível copiar o nome do grupo",
			color: "error",
			duration: 3000,
		});
	}
...
// Limpa o input quando o grupo muda
watch(
	() => props.grupo,
	() => {
		confirmacaoNome.value = "";
...
<template>
	<UiModal
		...
		@close="handleClose"
	>
		<div v-if="grupo" class="space-y-4">
			...
			<div class="flex items-start gap-3">
				...
				>
					<Icon name="lucide:layers" class="w-8 h-8 text-[var(--text-muted)]" />
				</div>
				...
				<div class="flex-1 min-w-0">
					...
					<p v-if="grupo.descricao" class="text-sm text-[var(--text-muted)] mt-1 line-clamp-2">
						{{ grupo.descricao }}
					</p>
					<div class="flex items-center gap-4 mt-2 text-xs text-[var(--text-muted)]">
						<span class="flex items-center">
							<Icon name="lucide:tag" class="w-3 h-3 mr-1" />
							{{ grupo?.adicionais_count ?? 0 }} adicional(is)
						</span>
						<span class="flex items-center">
							<Icon
								:name="grupo.ativo ? 'lucide:check-circle' : 'lucide:x-circle'"
								class="w-3 h-3 mr-1"
							/>
							{{ grupo.status_display }}
						</span>
					</div>
				</div>
			</div>
			...
			<div class="space-y-2">
				...
				<div class="space-y-2">
					...
					<p v-if="confirmacaoNome && !isConfirmacaoValida" class="text-xs text-[var(--error)]">
						O nome digitado não corresponde ao nome do grupo
					</p>
				</div>
			</div>
		</div>
